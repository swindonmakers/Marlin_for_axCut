...
void process_commands()
{
  ...
  int fadeLevel, fadeDuration;  // throw away
  float newLaserPower;  // throw away
  char *starpos = NULL;
#ifdef ENABLE_AUTO_BED_LEVELING
  ...
  else if(code_seen('M'))
  {
    ...
    case 4:    // M4 - turn laser on, power level as percentage as S parameter (range 0-100)
	  if (isLaserArmed()) {
        newLaserPower = getLaserPower();
        if(code_seen('S')) newLaserPower = code_value();
    ...
    case 680:  // M680 - Turn relay on, P is relay number from 1-4
        if(code_seen('P'))
        {
          turnRelayOn(code_value());
        }
	    break;
    case 681:  // M681 - Turn relay off, P is relay number from 1-4
        if(code_seen('P'))
        {
          turnRelayOff(code_value());
        }
	    break;
    case 682:   // M682 - Set lighting level, S 0-255
        if(code_seen('S'))
        {
          setLighting(code_value());
        }
        break;
    case 683:   // M683 - Fade lights to level
        fadeLevel = 255;
        fadeDuration = 1000;
        if(code_seen('S')) fadeLevel = code_value();
        if(code_seen('P')) fadeDuration = code_value();
        fadeLighting(fadeLevel, fadeDuration);
        break;
